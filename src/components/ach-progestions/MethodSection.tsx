...
export const MethodSection: React.FC = () => {
  ...
  return (
    <section id="methode" className="py-28 md:py-36 bg-[#FAF9F7] scroll-mt-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7 relative">
          {steps.map((step, idx) => {
            ...
            return (
              ...
              <div
                key={step.number}
                className="p-2 rounded-[2.25rem] bg-slate-900/[0.02] ring-1 ring-slate-900/5 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1.5 hover:shadow-[0_20px_45px_-12px_rgba(0,0,0,0.08)]"
              >
                {/* Inner Core */}
                <div className="rounded-[calc(2.25rem-0.5rem)] bg-white p-7 sm:p-8 h-full flex flex-col items-center text-center border border-slate-100 shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)] relative group">
                  
                  {/* Step Number Badge */}
                  <span className="text-3xl font-black text-slate-200 group-hover:text-[#5E1449]/30 transition-colors tabular-nums mb-4">
                    {step.number}
                  </span>

                  {/* Dual Pod: Kitbitz Hand-Drawn Spot Illustration + Step Icon */}
                  <div className="relative mb-5">
                    <div className="w-16 h-16 rounded-2xl bg-[#FAF9FC] border border-slate-100 flex items-center justify-center p-2.5 group-hover:scale-110 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] shadow-2xs">
                      <Image
                        src={step.kitbitzImage}
                        alt={step.kitbitzAlt}
                        width={40}
                        height={40}
                        className="object-contain drop-shadow-2xs group-hover:-translate-y-0.5 transition-transform"
                      />
                    </div>
                    <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-white shadow-xs border border-slate-100 flex items-center justify-center ${step.iconColor}`}>
                      <Icon className="w-3 h-3 stroke-[2.5]" />
                    </div>
                  </div>

                  {/* Title */}
                  <h3 className="text-lg font-bold text-slate-900 mb-2.5 [text-wrap:balance]">
                    {step.title}
                  </h3>
