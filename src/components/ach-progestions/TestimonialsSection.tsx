...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section id="avis" className="py-16 sm:py-24 md:py-36 bg-[#FAF9F7] scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 sm:mb-16 gap-5 sm:gap-6">
          ...
          <div className="p-1 sm:p-1.5 rounded-2xl bg-slate-900/[0.03] ring-1 ring-slate-900/5 shadow-xs shrink-0 self-start md:self-auto">
            <div className="flex items-center gap-3.5 sm:gap-4 bg-white border border-slate-100 px-4 py-2.5 sm:px-5 sm:py-3 rounded-[calc(1rem-0.25rem)] shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]">
              <span className="text-2xl sm:text-3xl font-black text-[#FF6B00] tracking-tight tabular-nums">
                5,0/5
              </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {testimonials.map((item) => (
            ...
            >
              {/* Inner Core */}
              <div className="rounded-[calc(1.75rem-0.375rem)] sm:rounded-[calc(2rem-0.5rem)] bg-white p-5 sm:p-7 h-full flex flex-col justify-between border border-slate-100 shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]">
                <div>
                  {/* 5 Stars */}
                  <div className="flex items-center gap-1 text-[#FF6B00] mb-3 sm:mb-4">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" />
                    ))}
                  </div>
                ...
                {/* Verified badge */}
                <div className="mt-5 sm:mt-6 pt-3.5 sm:pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-semibold">
        ...
        <div className="mt-10 sm:mt-14 text-center">
          <a
            ...
            className="group inline-flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-[#5E1449] transition-colors py-2 px-4 rounded-full border border-slate-200/60 bg-white shadow-2xs hover:shadow-xs"
          >
